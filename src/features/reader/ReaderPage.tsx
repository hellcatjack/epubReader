...
const ttsSentenceNoteReadingMaximumWidthPx = 640;
const ttsSentenceNoteReadingMinimumWidthPx = 320;
...
export function resolveTtsSentenceNotePlacement(args: {
  ...
  const readingWidth = Math.max(0, readingRect.right - readingRect.left);
  const availableWidth = Math.max(0, stageRect.width - ttsSentenceNoteGapPx * 2);
  const width = Math.min(
    ttsSentenceNoteReadingMaximumWidthPx,
    availableWidth,
    Math.max(ttsSentenceNoteReadingMinimumWidthPx, readingWidth),
  );
  ...
  const relativeActiveCenter = (activeRect.top + activeRect.bottom) / 2 - stageRect.top;
  const top =
    relativeActiveCenter < stageRect.height / 2
      ? maxTop
      : ttsSentenceNoteTopPaddingPx;
